...
io.on("connection", (socket) => {
    socket.on("joystick", (lr, x, y, c) => {
        controllerData[lr].joystick = [x, y, c]
        updateFile(lr);
    });

    socket.on("touchpad", (lr, x, y, c) => {
        controllerData[lr].touchpad = [x, y, c]
        updateFile(lr);
    });
...
function updateFile(hlr) {
    ...
    if (hlr === "h") {
        ...
        newline = newline.replaceAll("undefined", "0");
        console.log(newline)
